...
cap = cv2.VideoCapture('coldplay.avi')
ret, frame = cap.read()
while ret != True:
    ret,frame = cap.read()
...
cg = np.copy(bg)
gr = cv2.cvtColor(bg,cv2.COLOR_BGR2GRAY)
...
kernel_keys = np.ones((30,2),np.uint8)
...
while ret == True:
    ...
    if seconds_counter%60 == 0:
        print seconds_counter/60
    gr = cv2.cvtColor(frame,cv2.COLOR_BGR2GRAY)
    gray_blur = cv2.blur(gr,(3,3))

    ####### Mask operations (MOST IMPORTANT)################
    #fgmask = cv2.absdiff(bg,gray_blur)
    cgmask = cv2.absdiff(cg,frame)
    fgmask = cv2.blur(cgmask,(2,2))
    fgmask = cv2.cvtColor(fgmask,cv2.COLOR_BGR2HLS)
    fgmask = cv2.inRange(fgmask,np.array([0,100,0]),np.array([179,255,100])) #50 is toooo low!
    ...
    volumes_per_frame.append(notes_pressed)
    notes_pressed = [k for k,v in notes_pressed.iteritems()]
    #NUMBER IS ARBITRARY, ADJUST

    notes_per_frame.append(notes_pressed)
      
    cv2.imshow('orig',cgmask)
